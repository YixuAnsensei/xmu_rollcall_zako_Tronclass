...
V3 在**完整保留 V2.0 数字签到全部功能**的基础上，融合了雷达签到引擎（四校区两阶段定位）：

- **主页与课程列表完全不变**：依然是猫爪 → 登录 → 课程列表的单线流程，数字签到逻辑零改动。
- **课程详情页智能判定**：点进一门课后，自动检查最新签到记录的类型：
  - 最新是**数字签到** → 和以前一样显示 4 位签到码；
  - **雷达签到正在进行** → 显示「📡 雷达签到进行中喵❤」+ 一键雷达签到按钮；
  - 只有**历史雷达记录** → 显示「上一次是雷达签到喵❤」及签到时间。
- **一键雷达签到**：点击按钮后自动执行「四校区探针锁定 → 校区内三边定位反解教师坐标 → 提交签到」，全程进度条 + 实时日志可视化，失败可一键重试。
- **雷达引擎原理**：畅课雷达签到接口由学生端自报经纬度，服务端校验与教师位置的距离；失败响应会回传 `distance` 字段。V3 把接口当成“距离预言机”，先在四个校区中心各打一次探针锁定校区，再用两圆交点三边定位反解出教师位置，一击即中。

> ⚠️ **V3.0 目前是预览版（Preview）**：雷达签到部分尚未经过真实课堂的端到端验证，欢迎测试反馈 Issue 喵！追求稳定请继续使用「旧版备份V1」release。
...
4. **第三步：获取结果**：
   ...
   - **V3 预览版**：如果该课正在进行雷达签到，会显示「📡 雷达签到进行中」按钮，点击即可自动定位签到喵。
5. **查看日志（可选）**：
   - 如果你想看后台到底发生了什么，点击右上角的 **`📋 日志`**。所有的抓包细节、API 请求日志都会在这里实时滚动显示。
