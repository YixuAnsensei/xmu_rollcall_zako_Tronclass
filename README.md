...
6. **`submit_number_code()` —— 自动提交（V4 新增）**
   - **核心功能**：复用取码链路获得 `number_code` 后，构造 `{"deviceId": uuid4, "numberCode": code}` 载荷调用 `PUT /api/rollcall/{id}/answer_number_rollcall` 完成签到提交，内置「已结束拒绝提交」与「无码拒绝提交」双重守卫。
...
### 🎉 V4.0 预览版 (`zako_app_V3.0.py`) —— 新增一键数字签到自动提交！

V4 在**完整保留 V3.0 双引擎（数字查码 + 雷达签到）**的基础上，补全了数字签到的"最后一公里"——不仅能看到签到码，还能一键帮你提交签到：

- **🐾 一键数字签到**：课程详情的数字签到卡片新增「🐾 一键数字签到」按钮，与雷达签到按钮完全对称，点击后自动取码并调用 `answer_number_rollcall` 接口完成提交，全程可视化进度与结果反馈；
- **智能状态守卫**：仅当签到处于「进行中」状态才展示提交按钮；已结束的签到明确提示「无需提交」，杜绝无效请求；
- **提交异常全分类**：未获取到签到码、签到已截止、网络异常、HTTP 失败四类场景分别给出明确提示，支持一键无损重试与返回签到码视图；
- **端到端全链路模拟验证**：本地高保真 mock 服务器实测数字签到（取码 → 提交 → 服务端确认）与雷达签到（四校区探针 → 三边解算 → 命中教师坐标 0.0m）两条链路真实签到成功。
...
1. **启动**：
   - **V4 预览版**：双击运行 `zako_rollcall_V4_preview.exe`。
...
3. **第二步：选择课程**：
   ...
   - 点击你想要查询或签到的目标课程。
4. **第三步：获取结果与签到**：
   - **数字签到**：程序展示最新的 **4 位签到码**、**发起时间**以及**当前状态**（进行中/已结束）；若签到正在进行，点击「**🐾 一键数字签到**」按钮，程序自动取码提交完成签到喵。
...
| 版本 | 内容 | 适用人群 |
| ---- | ---- | -------- |
| [**V4.0 预览版**](https://github.com/YixuAnsensei/xmu_rollcall_zako_Tronclass/releases/tag/V4.0-Preview) | 数字签到查码 + 一键数字提交 + 雷达签到（最新） | 想体验完整自动签到闭环的 XMUer |
| [**V3.0 预览版**](https://github.com/YixuAnsensei/xmu_rollcall_zako_Tronclass/releases/tag/V3.0-Preview) | 数字签到查码 + 雷达签到 | 已习惯 V3 查码流程的用户 |
